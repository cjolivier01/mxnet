...
static float EvaluateTune(bool verbose = true) {
  ...
  if (test::performance_run) {
    binary_operators = {
      {"broadcast_add", COREOP_BWD_OP_NAME_VALUE_NONE},
      {"broadcast_mul", COREOP_BWD_OP_NAME_VALUE_NONE},
      {"broadcast_div", COREOP_BWD_OP_NAME_VALUE_NONE}
    };
  } else {
    binary_operators = {
      {"broadcast_add", COREOP_BWD_OP_NAME_VALUE_NONE}
  ...
  for (size_t i = 0, n = binary_operators.size(); i < n; ++i) {
    ...
    std::cout << "Operators: " << binary_operators[i].first << ", "
              << binary_operators[i].second << " for type: " << test::type_name<DType>()
              << std::endl;
    std::cout << "******************************" << std::endl;
